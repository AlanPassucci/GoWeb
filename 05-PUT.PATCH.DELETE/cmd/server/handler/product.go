...
type BodyRequestUpdateProduct struct {
	Name        string  `json:"name"`
	Quantity    int     `json:"quantity"`
	CodeValue   string  `json:"code_value"`
	IsPublished bool    `json:"is_published"`
...
type BodyRequestPartialUpdateProduct struct {
	Name        string  `json:"name"`
	Quantity    int     `json:"quantity"`
	CodeValue   string  `json:"code_value"`
	IsPublished bool    `json:"is_published"`
...
func (h *ProductHandler) UpdateProduct() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		var body BodyRequestUpdateProduct
		var m map[string]any
		if err := ctx.ShouldBindJSON(&m); err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{"error": "invalid request body"})
			return
		...
		updatedProduct, err := h.s.Update(p, false)
		if err != nil {
			switch {
			case err == product.ErrInvalidName || err == product.ErrInvalidCodeValue || err == product.ErrInvalidQuantity || err == product.ErrInvalidExpiration || err == product.ErrInvalidPrice:
				ctx.JSON(http.StatusBadRequest, gin.H{
		...
		updatedProductJSON := ConvertProductToProductJSON(updatedProduct)
		ctx.JSON(http.StatusOK, updatedProductJSON)
	}
}

func (h *ProductHandler) PartialUpdateProduct() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		productID, err := strconv.Atoi(ctx.Param("id"))
		if err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{
		...
		body := BodyRequestPartialUpdateProduct{
			Name:        productFound.Name,
			Quantity:    productFound.Quantity,
			CodeValue:   productFound.CodeValue,
			IsPublished: productFound.IsPublished,
			Expiration:  productFound.Expiration,
		...
		updatedProduct, err := h.s.Update(p, true)
		if err != nil {
			switch {
			case err == product.ErrInvalidName || err == product.ErrInvalidQuantity || err == product.ErrInvalidExpiration || err == product.ErrInvalidPrice:
				ctx.JSON(http.StatusBadRequest, gin.H{
					"error": err.Error(),
				})
				return
...
func (h *ProductHandler) DeleteProduct() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if err := h.s.Delete(productID); err != nil {
			ctx.JSON(http.StatusNotFound, gin.H{
				"error": err.Error(),
			})
			return
